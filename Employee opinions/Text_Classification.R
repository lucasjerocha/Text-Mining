...
opinion_split <- initial_split(opinions, strata = "assessment", prop = 0.8) # 80% training and 20% testing
train_data <- opinion_split %>% 
  training()
test_data <- opinion_split %>%
  testing()

# Data preprocessing with recipe

data_rec <- recipe(formula = assessment ~ comment, data = train_data) %>% # The testing data should remain untouched
  step_filter(comment != "") %>%
  step_tokenize(comment) %>%
  step_stopwords(comment, language = "en", keep = FALSE) %>%
  step_tokenfilter(comment, max_times = 200, min_times = 2) %>% # Removing infrequent or overly frequent terms 
  step_tf(comment) %>% # Transforming tokens into Term-Frequencies
...
final_model <- glm_model %>%
  fit(assessment ~ ., data = train_baked) # Specifies the formula for the model; assessment is the output variable

# Testing

predictions_glm <- final_model %>%
  predict(new_data = test_baked) %>% # The data on which we want to make predictions
  bind_cols(test_baked %>% select(assessment)) # To compare the model predictions (test_baked) with the actual labels of the test data
predictions_glm

predictions_glm %>%
  conf_mat(assessment, .pred_class) # Confusion matrix

predictions_glm %>%
  metrics(assessment, .pred_class) %>% # To calculate important metrics as precision, recall, AUC-ROC, etc
  select(-.estimator) %>%
...
data_rec2 <- recipe(formula = assessment ~ comment, data = train_data) %>%
  step_filter(comment != "") %>% 
  step_tokenize(comment, token = "ngrams", options = list(n = 2)) %>%
  step_stopwords(comment, language = "en", keep = FALSE) %>%
  step_tokenfilter(comment, max_times = 200, min_times = 2) %>%
  step_tf(comment) %>%
...
final_model2 <- glm_model2 %>%
  fit(assessment ~., data = train_baked2)

# Testing

predictions_glm2 <- final_model2 %>%
  predict(new_data = test_baked2) %>%
  bind_cols(test_baked2 %>% select(assessment))
...
data_rec3 <- recipe(formula = assessment ~ comment, data = train_data) %>%
  step_filter(comment != "") %>% 
  step_tokenize(comment) %>%
  step_stopwords(comment, language = "en", keep = FALSE) %>%
  step_tokenfilter(comment, max_times = 200, min_times = 2) %>%
  step_tfidf(comment) %>% # The contrast with model 1
  prep(training = train_data)
data_rec3
...
final_model3 <- glm_model3 %>%
  fit(assessment ~ ., data = train_baked3)

# Testing

predictions_glm3 <- final_model3 %>%
  predict(new_data = test_baked3) %>%
  bind_cols(test_baked3 %>% select(assessment))
...
opinion_split <- initial_split(opinions, strata = "assessment", prop = 0.8)
...
data_rec <- recipe(formula = assessment ~ comment, data = train_data) %>%
  ...
  step_tokenize(comment) %>%
  step_stopwords(comment, language = "en", keep = FALSE) %>%
  step_word_embeddings(comment, embeddings = lexicon_score) %>% # The function calculates word embeddings based on the sentiment scores
  prep(training = train_data)
data_rec
